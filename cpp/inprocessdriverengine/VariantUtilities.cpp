...
bool VariantUtilities::VariantIsString(const VARIANT& value) {
  return value.vt == VT_BSTR;
}

bool VariantUtilities::VariantIsInteger(const VARIANT& value) {
  return value.vt == VT_I4 || value.vt == VT_I8;
}

bool VariantUtilities::VariantIsDouble(const VARIANT& value) {
  return value.vt == VT_R4 || value.vt == VT_R8;
}

bool VariantUtilities::VariantIsBoolean(const VARIANT& value) {
  return value.vt == VT_BOOL;
}

bool VariantUtilities::VariantIsEmpty(const VARIANT& value) {
  return value.vt == VT_EMPTY;
}

bool VariantUtilities::VariantIsIDispatch(const VARIANT& value) {
  return value.vt == VT_DISPATCH;
}

bool VariantUtilities::VariantIsElementCollection(const VARIANT& value) {
...
bool VariantUtilities::VariantIsElement(const VARIANT& value) {
  if (value.vt == VT_DISPATCH) {
...
bool VariantUtilities::VariantIsObject(const VARIANT& value) {
  if (value.vt != VT_DISPATCH) {
    return false;
  }
...
int VariantUtilities::VariantAsJsonValue(const VARIANT& variant_value,
                                         ElementRepository* element_manager,
                                         Json::Value* value) {
  std::vector<IDispatch*> visited;
  if (HasSelfReferences(variant_value, &visited)) {
    return EUNEXPECTEDJSERROR;
  }
  return ConvertVariantToJsonValue(variant_value, element_manager, value);
}

bool VariantUtilities::HasSelfReferences(const VARIANT& current_object,
                                         std::vector<IDispatch*>* visited) {
...
int VariantUtilities::ConvertVariantToJsonValue(
    ...
    *value = Json::Value::null;
    ...
    if (VariantIsArray(variant_value) ||
      ...
      for (long i = 0; i < length; ++i) {
        ...
        ConvertVariantToJsonValue(array_item,
                                  element_manager,
                                  &array_item_result);
      ...
      *value = result_array;
      ...
      if (ExecuteToJsonMethod(variant_value, &json_serialized)) {
        ConvertVariantToJsonValue(json_serialized,
                                  element_manager,
                                  &result_object);
      } else {
        int property_enum_status =
            GetAllVariantObjectPropertyValues(variant_value,
                                              element_manager,
                                              &result_object);
      ...
      *value = result_object;
      ...
      if (FAILED(hr)) {
        ...
        if (ExecuteToJsonMethod(variant_value, &json_serialized_variant)) {
          ...
          ConvertVariantToJsonValue(json_serialized_variant,
                                    element_manager,
                                    &interim_value);
          *value = interim_value;
          return WD_SUCCESS;
...
bool VariantUtilities::ExecuteToJsonMethod(const VARIANT& object_to_serialize,
                                           VARIANT* json_object_variant) {
  CComVariant to_json_method;
  bool has_to_json_property =
      GetVariantObjectPropertyValue(object_to_serialize.pdispVal,
                                    L"toJSON",
                                    &to_json_method);
...
bool VariantUtilities::GetVariantObjectPropertyValue(
    IDispatch* variant_object_dispatch,
    const std::wstring& property_name,
    VARIANT* property_value) {
  LPOLESTR property_name_pointer = reinterpret_cast<LPOLESTR>(
      const_cast<wchar_t*>(property_name.data()));
...
std::wstring VariantUtilities::GetVariantObjectTypeName(const VARIANT& value) {
  std::wstring name = L"";
  if (value.vt == VT_DISPATCH && value.pdispVal) {
    CComPtr<ITypeInfo> typeinfo;
    HRESULT get_type_info_result =
        value.pdispVal->GetTypeInfo(0,
                                    LOCALE_USER_DEFAULT,
                                    &typeinfo);
    TYPEATTR* type_attr;
    CComBSTR name_bstr;
...
int VariantUtilities::GetPropertyNameList(
    IDispatch* object_dispatch,
    std::vector<std::wstring>* property_names) {
...
int VariantUtilities::GetArrayLength(IDispatch* array_dispatch,
                                     long* length) {
...
int VariantUtilities::GetAllVariantObjectPropertyValues(
    const VARIANT& variant_value,
    ElementRepository* element_manager,
    Json::Value* value) {
  std::vector<std::wstring> property_names;
  ...
  for (size_t i = 0; i < property_names.size(); ++i) {
    ...
    ConvertVariantToJsonValue(property_value_variant,
                              element_manager,
                              &property_value);
