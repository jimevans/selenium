...
int SendInputActionSimulator::SimulateActions(ActionContext context,
  ...
  for (; it != sleep_indexes.end(); ++it) {
    ...
    if (number_of_inputs > 0) {
      this->SendInputToBrowser(window_handle,
                               inputs,
...
void SendInputActionSimulator::SendInputToBrowser(HWND window_handle,
                                                  std::vector<INPUT> inputs,
                                                  int start_index,
                                                  int input_count) {
  if (input_count > 0) {
