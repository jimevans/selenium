...
#include "ActionSimulators/ActionContext.h"
...
int InputManager::PerformInputSequence(InputContext context,
  ...
  for (size_t i = 0; i < ticks.size(); ++i) {
    ...
    for (size_t j = 0; j < tick.size(); ++j) {
      ...
      if (action_subtype == "pointerMove") {
        status_code = this->PointerMoveTo(context, action, &current_input_state);
      } else if (action_subtype == "pointerDown") {
        status_code = this->PointerDown(context, action, &current_input_state);
      } else if (action_subtype == "pointerUp") {
        status_code = this->PointerUp(context, action, &current_input_state);
      } else if (action_subtype == "keyDown") {
        status_code = this->KeyDown(context, action, &current_input_state);
      } else if (action_subtype == "keyUp") {
        status_code = this->KeyUp(context, action, &current_input_state);
      } else if (action_subtype == "pause") {
        status_code = this->Pause(context, action);
      }
  ...
  if (this->inputs_.size() > 0) {
    ...
    action_context.document = context.document;
    action_context.window_handle = context.window_handle;
    this->action_simulator_->SimulateActions(action_context,
                                             this->inputs_,
...
int InputManager::PointerMoveTo(InputContext context,
  ...
  if (this->action_simulator_->UseExtraInfo()) {
    ...
    if (element_specified) {
      ...
      // calculated here.
      bool click_point_reachable = target_element->GetClickableLocationNoScroll(&element_location);
      ...
      //  }
      //}
      if (!click_point_reachable) {
        return EELEMENTCLICKPOINTNOTSCROLLED;
      }
...
int InputManager::PointerDown(InputContext context,
...
int InputManager::PointerUp(InputContext context,
...
int InputManager::KeyDown(InputContext context,
                          const Json::Value& down_action,
                          InputState* input_state) {
  int status_code = WD_SUCCESS;
...
int InputManager::KeyUp(InputContext context,
                        const Json::Value& up_action,
                        InputState* input_state) {
  int status_code = WD_SUCCESS;
...
int InputManager::Pause(InputContext context,
  ...
  if (duration > 0) {
    this->AddPauseInput(context.window_handle, duration);
  }
  return status_code;
}
