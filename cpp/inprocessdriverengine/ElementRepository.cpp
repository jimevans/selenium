...
int ElementRepository::GetManagedElement(
    const std::string& element_id,
    ElementHandle* element_wrapper) const {
  ElementMap::const_iterator found_iterator =
      this->managed_elements_.find(element_id);
...
bool ElementRepository::AddManagedElement(IHTMLElement* element,
                                          ElementHandle* element_wrapper) {

  bool element_already_managed = this->IsElementManaged(element,
                                                        element_wrapper);
...
void ElementRepository::RemoveManagedElement(const std::string& element_id) {
  ElementMap::iterator found_iterator =
      this->managed_elements_.find(element_id);
  if (found_iterator != this->managed_elements_.end()) {
