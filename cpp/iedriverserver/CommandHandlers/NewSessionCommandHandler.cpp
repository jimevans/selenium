...
void NewSessionCommandHandler::ExecuteInternal(
  ...
  if (it != command_parameters.end()) {
    ...
    Json::Value validated_capabilities =
        this->ValidateArguments(it->second, &error_message);
...
Json::Value NewSessionCommandHandler::GetCapability(
  ...
  Json::Value capability_value = capabilities.get(capability_name,
                                                  default_value);
...
bool NewSessionCommandHandler::IsEquivalentType(
  ...
  if ((expected_type == Json::intValue ||
        expected_type == Json::uintValue ||
        expected_type == Json::realValue) &&
      (actual_type == Json::intValue ||
        actual_type == Json::uintValue ||
        actual_type == Json::realValue)) {
...
Json::Value NewSessionCommandHandler::ValidateArguments(const Json::Value& capabilities,
  ...
  if (this->ValidateCapabilities(always_match,
                                  "alwaysMatch",
                                  error_message)) {
    ...
    if (!first_match_candidates.isArray()) {
      *error_message = "firstMatch must be a JSON list";
      ...
      for (size_t i = 0; i < first_match_candidates.size(); ++i) {
        LOG(DEBUG) << "Validating firstMatch capability set with index "
                    << i;
        std::string first_match_validation_error = "";
        Json::Value first_match_candidate =
            first_match_candidates[static_cast<int>(i)];
      ...
      if (validated_first_match_candidates.size() > 0) {
        ...
        *error_message = "";
        ...
        for (size_t i = 0;
              i < validated_first_match_candidates.size();
              ++i) {
          Json::Value merged_capabilities(Json::objectValue);
          Json::Value first_match =
              validated_first_match_candidates[static_cast<int>(i)];
...
Json::Value NewSessionCommandHandler::ProcessCapabilities(
    const IESession& executor,
    const Json::Value& capabilities,
    std::string* error_message) {
  ...
  for (size_t i = 0; i < capabilities.size(); ++i) {
    ...
    if (this->MatchCapabilities(executor,
                                merged_capabilities,
                                &match_error)) {
      ...
      Json::Value unexpected_alert_behavior =
          this->GetCapability(merged_capabilities,
                              UNHANDLED_PROMPT_BEHAVIOR_CAPABILITY,
                              Json::stringValue,
                              Json::Value(Json::stringValue));
      ::SendMessage(
          executor.session_settings_window_handle(),
          WD_SET_SESSION_SETTING,
          SESSION_SETTING_UNHANDLED_PROMPT_BEHAVIOR,
          reinterpret_cast<LPARAM>(&unexpected_alert_behavior.asString()));
      ...
      ::SendMessage(
          ...
          reinterpret_cast<LPARAM>(&page_load_strategy.asString()));
      ...
      Json::Value use_strict_file_interactability =
          this->GetCapability(merged_capabilities,
                              ...
                              Json::booleanValue,
                              false);
      bool interactability_value = use_strict_file_interactability.asBool();
      ::SendMessage(
          executor.session_settings_window_handle(),
          WD_SET_SESSION_SETTING,
          SESSION_SETTING_STRICT_FILE_INTERACTABLILITY,
          reinterpret_cast<LPARAM>(&interactability_value));
      ...
      if (merged_capabilities.isMember(PROXY_CAPABILITY)) {
        Json::Value use_per_process_proxy_capability =
            this->GetCapability(ie_options,
                                USE_PER_PROCESS_PROXY_CAPABILITY,
                                Json::booleanValue,
                                false);
        bool use_per_process_proxy =
            use_per_process_proxy_capability.asBool();
...
void NewSessionCommandHandler::SetTimeoutSettings(
    const IESession& executor, const Json::Value& capabilities) {
  ...
  if (capabilities.isMember(IMPLICIT_WAIT_TIMEOUT_NAME)) {
    unsigned long long timeout_value =
        capabilities[IMPLICIT_WAIT_TIMEOUT_NAME].asUInt64();
    ::SendMessage(
        executor.session_settings_window_handle(),
        WD_SET_SESSION_SETTING,
        SESSION_SETTING_IMPLICIT_WAIT_TIMEOUT,
        reinterpret_cast<LPARAM>(&timeout_value));
  }
  if (capabilities.isMember(PAGE_LOAD_TIMEOUT_NAME)) {
    unsigned long long timeout_value =
        capabilities[PAGE_LOAD_TIMEOUT_NAME].asUInt64();
    ::SendMessage(
        executor.session_settings_window_handle(),
        WD_SET_SESSION_SETTING,
  ...
  if (capabilities.isMember(SCRIPT_TIMEOUT_NAME)) {
    long long timeout_value = -1;
    if (!capabilities[SCRIPT_TIMEOUT_NAME].isNull()) {
      timeout_value = capabilities[PAGE_LOAD_TIMEOUT_NAME].asInt64();
    }
    ::SendMessage(
        executor.session_settings_window_handle(),
        WD_SET_SESSION_SETTING,
...
void NewSessionCommandHandler::SetBrowserFactorySettings(const IESession& executor,
                                                          const Json::Value& capabilities) {
  LOG(TRACE) << "Entering NewSessionCommandHandler::SetBrowserFactorySettings";
  std::string default_initial_url =
      "http://localhost:" +
      std::to_string(static_cast<long long>(executor.port())) +
      "/";
  if (!capabilities.isNull()) {
    BrowserFactorySettings factory_settings;
    Json::Value ignore_protected_mode_settings =
        this->GetCapability(capabilities,
                            IGNORE_PROTECTED_MODE_CAPABILITY,
                            Json::booleanValue,
                            false);
    factory_settings.ignore_protected_mode_settings =
        ignore_protected_mode_settings.asBool();

    Json::Value ignore_zoom_setting =
        this->GetCapability(capabilities,
    ...
    Json::Value browser_attach_timeout =
        this->GetCapability(capabilities,
    ...
    Json::Value force_create_process_api =
        this->GetCapability(capabilities,
                            FORCE_CREATE_PROCESS_API_CAPABILITY,
                            Json::booleanValue,
                            false);
    factory_settings.force_create_process_api =
        force_create_process_api.asBool();

    Json::Value force_shell_windows_api =
        this->GetCapability(capabilities,
                            FORCE_SHELL_WINDOWS_API_CAPABILITY,
                            Json::booleanValue,
                            false);
    factory_settings.force_shell_windows_api =
        force_shell_windows_api.asBool();
    ...
    Json::Value browser_command_line_switches =
        this->GetCapability(capabilities,
                            ...
                            Json::stringValue,
                            Json::Value(Json::stringValue));
    factory_settings.browser_command_line_switches =
        browser_command_line_switches.asString();

    Json::Value ensure_clean_session = 
        this->GetCapability(capabilities,
                            ENSURE_CLEAN_SESSION_CAPABILITY,
                            Json::booleanValue,
                            false);
    factory_settings.clear_cache_before_launch =
        ensure_clean_session.asBool();
...
void NewSessionCommandHandler::SetProxySettings(
    const IESession& executor,
  ...
  if (!proxy_capability.isNull()) {
    ...
    std::string proxy_type = proxy_capability.get("proxyType",
                                                  "").asString();
    proxy_settings.proxy_type = proxy_type;
    std::string http_proxy = proxy_capability.get("httpProxy",
                                                  "").asString();
    ...
    std::string socks_proxy = proxy_capability.get("socksProxy",
                                                    "").asString();
    ...
    if (socks_proxy.length() > 0) {
      ...
      std::string socks_user_name = proxy_capability.get("socksUsername",
                                                          "").asString();
      proxy_settings.socks_user_name = socks_user_name;
      std::string socks_password = proxy_capability.get("socksPassword",
                                                        "").asString();
      proxy_settings.socks_password = socks_password;
    }
    std::string autoconfig_url = proxy_capability.get("proxyAutoconfigUrl",
                                                      "").asString();
    proxy_settings.proxy_autoconfig_url = autoconfig_url;

    Json::Value proxy_bypass_list = proxy_capability.get("noProxy", 
                                                          Json::Value::null);
    ...
    ::SendMessage(executor.session_settings_window_handle(),
                  WD_SET_SESSION_SETTING,
                  SESSION_SETTING_PROXY,
                  reinterpret_cast<LPARAM>(&proxy_settings));
  }
}

void NewSessionCommandHandler::SetInputSettings(
    const IESession& executor,
    const Json::Value& capabilities) {
...
Json::Value NewSessionCommandHandler::CreateReturnedCapabilities(
  ...
  capabilities[BROWSER_VERSION_CAPABILITY] = 
      std::to_string(static_cast<long long>(executor.browser_factory()->browser_version()));
  ...
  capabilities[PAGE_LOAD_STRATEGY_CAPABILITY] = page_load_strategy;

  bool strict_file_interactability = false;
  ::SendMessage(executor.session_settings_window_handle(),
                WD_GET_SESSION_SETTING,
                SESSION_SETTING_STRICT_FILE_INTERACTABLILITY,
                reinterpret_cast<LPARAM>(&strict_file_interactability));

  capabilities[STRICT_FILE_INTERACTABILITY_CAPABILITY] =
  ...
  if (unhandled_prompt_behavior.size() > 0) {
    capabilities[UNHANDLED_PROMPT_BEHAVIOR_CAPABILITY] =
        unhandled_prompt_behavior;
  } else {
    capabilities[UNHANDLED_PROMPT_BEHAVIOR_CAPABILITY] =
        DISMISS_AND_NOTIFY_UNEXPECTED_ALERTS;
  }
  ...
  timeouts[IMPLICIT_WAIT_TIMEOUT_NAME] = implicit_wait_timeout;
  unsigned long long page_load_timeout = 0;
  ::SendMessage(executor.session_settings_window_handle(),
                WD_GET_SESSION_SETTING,
                SESSION_SETTING_PAGE_LOAD_TIMEOUT,
                reinterpret_cast<LPARAM>(&page_load_timeout));
  timeouts[PAGE_LOAD_TIMEOUT_NAME] = page_load_timeout;
  long long script_timeout = -1;
  ::SendMessage(executor.session_settings_window_handle(),
                WD_GET_SESSION_SETTING,
                SESSION_SETTING_SCRIPT_TIMEOUT,
  ...
  ie_options[IGNORE_PROTECTED_MODE_CAPABILITY] =
      executor.browser_factory()->ignore_protected_mode_settings();
  ie_options[IGNORE_ZOOM_SETTING_CAPABILITY] =
      executor.browser_factory()->ignore_zoom_setting();
  ie_options[INITIAL_BROWSER_URL_CAPABILITY] =
      executor.browser_factory()->initial_browser_url();
  ie_options[BROWSER_ATTACH_TIMEOUT_CAPABILITY] =
      executor.browser_factory()->browser_attach_timeout();
  ie_options[BROWSER_COMMAND_LINE_SWITCHES_CAPABILITY] =
      executor.browser_factory()->browser_command_line_switches();
  ie_options[FORCE_CREATE_PROCESS_API_CAPABILITY] =
      executor.browser_factory()->force_createprocess_api();
  ...
  ::SendMessage(executor.session_settings_window_handle(),
                ...
                SESSION_SETTING_PROXY,
                reinterpret_cast<LPARAM>(&proxy_settings));
  if (proxy_settings.is_set) {
    ie_options[USE_PER_PROCESS_PROXY_CAPABILITY] =
        proxy_settings.use_per_process_proxy;
    capabilities[PROXY_CAPABILITY] = proxy_settings.AsJson();
  } else {
...
bool NewSessionCommandHandler::MatchCapabilities(
    const IESession& executor,
  ...
  std::vector<std::string> capability_names =
      merged_capabilities.getMemberNames();
  std::vector<std::string>::const_iterator name_iterator =
  ...
  for (; name_iterator != capability_names.end(); ++name_iterator) {
    ...
    if (capability_name == BROWSER_NAME_CAPABILITY &&
      ...
      *error_message =
          "browserName must be 'internet explorer', but was '" +
    ...
    if (capability_name == PLATFORM_NAME_CAPABILITY &&
      ...
      *error_message =
          "platformName must be 'windows', but was '" +
    ...
    if (capability_name == BROWSER_VERSION_CAPABILITY) {
      ...
      std::string requested_browser_version_value =
          merged_capabilities[BROWSER_VERSION_CAPABILITY].asString();
      int available_browser_version =
          executor.browser_factory()->browser_version();
      int requested_browser_version =
          atoi(requested_browser_version_value.c_str());
    ...
    if (capability_name.find(":") != std::string::npos &&
      ...
      *error_message = capability_name + 
                        " is an unknown extension capability for IE";
      return false;
    }
  }
...
bool NewSessionCommandHandler::MergeCapabilities(
  ...
  std::vector<std::string> primary_property_names =
      primary_capabilities.getMemberNames();
  for (size_t i = 0; i < primary_property_names.size(); ++i) {
    std::string property_name = primary_property_names[i];
    (*merged_capabilities)[property_name] =
        primary_capabilities[property_name];
  }

  std::vector<std::string> secondary_property_names =
      secondary_capabilities.getMemberNames();
  for (size_t i = 0; i < secondary_property_names.size(); ++i) {
    ...
    (*merged_capabilities)[property_name] =
        secondary_capabilities[property_name];
  }
  return true;
}
...
bool NewSessionCommandHandler::ValidateCapabilities(
  ...
  for (; name_iterator != capability_names.end(); ++name_iterator) {
    ...
    if (capability_name == ACCEPT_INSECURE_CERTS_CAPABILITY) {
      LOG(DEBUG) << "Found " << ACCEPT_INSECURE_CERTS_CAPABILITY
                  << " capability. Validating value type is boolean.";
    ...
    if (capability_name == STRICT_FILE_INTERACTABILITY_CAPABILITY) {
      LOG(DEBUG) << "Found " << STRICT_FILE_INTERACTABILITY_CAPABILITY
                  << " capability. Validating value type is boolean.";
      if (!this->ValidateCapabilityType(capabilities,
        capability_name,
        Json::ValueType::booleanValue,
    ...
    if (capability_name == UNHANDLED_PROMPT_BEHAVIOR_CAPABILITY) {
      LOG(DEBUG) << "Found " << UNHANDLED_PROMPT_BEHAVIOR_CAPABILITY
                  << " capability. Validating value type is string.";
      if (!this->ValidateCapabilityType(capabilities,
                                        capability_name,
                                        Json::ValueType::stringValue,
                                        &capability_error_message)) {
        *error_message = "Invalid capabilities in " +
        ...
        LOG(DEBUG) << "Validating " << UNHANDLED_PROMPT_BEHAVIOR_CAPABILITY
                    << " capability is a valid value.";
        std::string unhandled_prompt_behavior =
            capabilities[capability_name].asString();
    ...
    if (capability_name == PAGE_LOAD_STRATEGY_CAPABILITY) {
      std::string page_load_strategy = "";
      LOG(DEBUG) << "Found " << PAGE_LOAD_STRATEGY_CAPABILITY
        << " capability. Validating value type is string.";
      if (!this->ValidateCapabilityType(capabilities,
        capability_name,
        Json::ValueType::stringValue,
        ...
        *error_message = "Invalid capabilities in " +
        ...
        LOG(DEBUG) << "Validating " << PAGE_LOAD_STRATEGY_CAPABILITY
                    << " capability is a valid value.";
    ...
    if (capability_name == TIMEOUTS_CAPABILITY) {
      ...
      if (!this->ValidateCapabilityType(capabilities,
        ...
        *error_message = "Invalid capabilities in " +
        ...
        std::vector<std::string>::const_iterator timeout_name_iterator =
            timeout_names.begin();
        for (; 
             timeout_name_iterator != timeout_names.end();
             ++timeout_name_iterator) {
          ...
          if (timeout_name != SCRIPT_TIMEOUT_NAME ||
    ...
    if (capability_name == PROXY_CAPABILITY) {
      ...
      if (!this->ValidateCapabilityType(capabilities,
        ...
        *error_message = "Invalid capabilities in " +
        ...
        std::vector<std::string>::const_iterator proxy_setting_iterator =
            proxy_setting_names.begin();
        for (;
              proxy_setting_iterator != proxy_setting_names.end();
              ++proxy_setting_iterator) {
    ...
    if (capability_name == IE_DRIVER_EXTENSIONS_CAPABILITY) {
      LOG(DEBUG) << "Found " << IE_DRIVER_EXTENSIONS_CAPABILITY
                  << " capability. Validating value type is object.";
      if (!this->ValidateCapabilityType(capabilities,
                                        capability_name,
                                        Json::ValueType::objectValue,
                                        &capability_error_message)) {
