...
class NewSessionCommandHandler : public SessionCommandHandler {
public:
  NewSessionCommandHandler(void);
  virtual ~NewSessionCommandHandler(void);

protected:
  void ExecuteInternal(const IESession& executor,
...
private:
  ...
  std::string GetUnexpectedAlertBehaviorValue(
      const std::string& desired_value);

  std::string GetPageLoadStrategyValue(const std::string& desired_value);

  void SetBrowserFactorySettings(const IESession& executor,
                                const Json::Value& capabilities);
  void SetInputSettings(const IESession& executor,
                        const Json::Value& capabilities);
  void SetTimeoutSettings(const IESession& executor,
                          const Json::Value& capabilities);
  void SetProxySettings(const IESession& executor,
                        const Json::Value& proxy_capability,
                        const bool use_per_process_proxy);

  Json::Value ProcessCapabilities(const IESession& executor,
                                  const Json::Value& capabilities,
  ...
  bool MergeCapabilities(const Json::Value& primary_capabilities,
                         ...
                         std::string* error_message);
  bool MatchCapabilities(const IESession& executor,
                         const Json::Value& merged_capabilities,
                         std::string* error_message);

  Json::Value CreateReturnedCapabilities(const IESession& executor);
};
