...
void BrowserFactory::Initialize(BrowserFactorySettings settings) {
  ...
  this->browser_command_line_switches_ =
      StringUtilities::ToWString(settings.browser_command_line_switches);
  this->initial_browser_url_ =
      StringUtilities::ToWString(settings.initial_browser_url);
  ...
  if (this->ignore_protected_mode_settings_ ||
    ...
    if (process_id == NULL) {
      ...
      if (error_message->size() == 0) {
        std::string launch_api_name = use_createprocess_api ?
      ...
      int buffer_count = ::GetProcessImageFileName(proc_info.hProcess,
                                                   &image_buffer[0],
                                                   MAX_PATH);
      std::wstring full_image_path = &image_buffer[0];
      size_t last_delimiter = full_image_path.find_last_of('\\');
      std::string image_name = StringUtilities::ToString(
          full_image_path.substr(last_delimiter + 1,
                                 buffer_count - last_delimiter));
      LOG(DEBUG) << "Process with ID " << process_id
                 << " is executing " << image_name;
    }
...
void BrowserFactory::LaunchBrowserUsingIELaunchURL(
    PROCESS_INFORMATION* proc_info, std::string* error_message) {
...
bool BrowserFactory::IsCreateProcessApiAvailable() {
  ...
  if (this->ie_major_version_ >= 8) {
    ...
    // If CreateProcess() is used and TabProcGrowth != 0 IE will use differen
    // tab and frame processes.
  ...
  if (window_handle != NULL && this->oleacc_instance_handle_) {
    ...
    LPFNOBJECTFROMLRESULT object_pointer =
        reinterpret_cast<LPFNOBJECTFROMLRESULT>(
            ::GetProcAddress(this->oleacc_instance_handle_, "ObjectFromLresult"));
...
bool BrowserFactory::AttachToBrowser(ProcessWindowInfo* process_window_info,
  ...
  if (!this->force_shell_windows_api_) {
    ...
    attached = this->AttachToBrowserUsingActiveAccessibility(
        process_window_info,
        error_message);
    if (!attached) {
    ...
    if (SUCCEEDED(hr)) {
      ...
      if (provider) {
        ...
        if (SUCCEEDED(hr)) {
          ...
          hr = child_provider->QueryService(
              SID_SWebBrowserApp,
              IID_IWebBrowser2,
...
bool BrowserFactory::AttachToBrowserUsingShellWindows(
  ...
  clock_t end = clock() +
                (this->browser_attach_timeout_ / 1000 * CLOCKS_PER_SEC);
        ...
        if (SUCCEEDED(hr)) {
          ...
          if (process_window_info->hwndBrowser != NULL) {
            LOG(DEBUG) << "Found window handle "
                       ...
                       << process_window_info->dwProcessId;
            CComPtr<IWebBrowser2> browser;
            hr = shell_window_variant.pdispVal->
                QueryInterface<IWebBrowser2>(&browser);
            if (FAILED(hr)) {
...
int BrowserFactory::GetZoomLevel(IHTMLDocument2* document,
                                 IHTMLWindow2* window) {
  ...
  if (this->ie_major_version_ == 7) {
    ...
    zoom = static_cast<int>(
        (static_cast<double>(right - left) / offset_width) * 100.0);
    ...
    zoom = static_cast<int>(
        (static_cast<double>(device_xdpi) / logical_xdpi) * 100.0);
  } else {
    // IE6 case
    zoom = 100;
  }
...
BOOL CALLBACK BrowserFactory::FindChildWindowForProcess(HWND hwnd,
                                                        LPARAM arg) {
  ProcessWindowInfo *process_window_info =
      reinterpret_cast<ProcessWindowInfo*>(arg);
...
BOOL CALLBACK BrowserFactory::FindDialogWindowForProcess(HWND hwnd,
                                                         LPARAM arg) {
...
void BrowserFactory::GetExecutableLocation() {
  ...
  if (RegistryUtilities::GetRegistryValue(HKEY_LOCAL_MACHINE,
    ...
    if (RegistryUtilities::GetRegistryValue(HKEY_LOCAL_MACHINE,
      ...
      // path.
      DWORD expanded_location_size =
          ::ExpandEnvironmentStrings(executable_location.c_str(), NULL, 0);
      std::vector<wchar_t> expanded_location(expanded_location_size);
      ::ExpandEnvironmentStrings(executable_location.c_str(),
                                 &expanded_location[0],
                                 expanded_location_size);
      ...
      if (arg_start_pos != std::string::npos) {
        this->ie_executable_location_ =
            executable_location.substr(0, arg_start_pos);
      }
      if (this->ie_executable_location_.substr(0, 1) == L"\"") {
        this->ie_executable_location_.erase(0, 1);
        this->ie_executable_location_.erase(
            this->ie_executable_location_.size() - 1,
            1);
      }
    } else {
...
void BrowserFactory::GetIEVersion() {
  ...
  std::string ie_version = FileUtilities::GetFileVersion(
      this->ie_executable_location_);
  
  if (ie_version.size() == 0) {
    // 64-bit Windows 8 has a bug where it does not return the executable
    // location properly
...
int BrowserFactory::GetZoneProtectedModeSetting(
    const HKEY key_handle,
  ...
  if (ERROR_SUCCESS == ::RegOpenKeyEx(key_handle,
    ...
    if (zone_subkey_name == ZONE_INTERNET ||
        ...
        (zone_subkey_name == ZONE_LOCAL_INTRANET && 
         this->ie_major_version_ == 7)) {
